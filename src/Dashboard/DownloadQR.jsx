...
const DownloadQR = () => {
  ...
  useEffect(() => {
    ...
    const generateQR = async () => {
      ...
      try {
        ...
        const url = new URL(`${process.env.REACT_APP_API}/auth/generate-qr/${id}`);
  ...
  const handleDownload = async () => {
    try {
      ...
      link.href = dataUrl;
      link.download = `xopy-qrcode-${userName.toLowerCase().replace(/\s+/g, '-')}.png`;
        ...
        <Card className="shadow-lg">
          <CardHeader className="text-center border-b pb-6">
            <div className="mx-auto mb-2">
              <Badge variant="outline" className="bg-yellow-100 hover:bg-yellow-100 text-yellow-800 border-yellow-300">
                QR Code
              </Badge>
            </div>
            <CardTitle className="text-2xl font-bold">Your Xopy QR Code</CardTitle>
          </CardHeader>
                ...
                >
                  ...
                  <div className="flex items-center justify-center mb-6">
                    <Avatar className="h-10 w-10 bg-yellow-500 mr-2">
                      <AvatarFallback className="font-bold text-white">X</AvatarFallback>
                    </Avatar>
                    <span className="text-2xl font-bold text-gray-800">Xopy</span>
                  </div>
                  ...
                  <div className="text-center mt-6">
                    ...
                    <div className="mt-4 flex items-center justify-center gap-2 bg-gray-50 py-3 px-4 rounded-md">
                      ...
                      <span className="text-gray-700">Scan & share files for printing</span>
                    </div>
                    
                    <div className="flex items-center justify-center gap-2 mt-2">
                      <Printer className="w-4 h-4 text-gray-400" />
                      <span className="text-sm text-gray-500">Ready for instant printing</span>
                    </div>
                  </div>

                  {/* Footer */}
                  <div className="mt-6 pt-4 border-t border-gray-200 text-center">
                    <p className="text-xs text-gray-500">Powered by Xopy • xopy.com</p>
                  </div>
                </div>
              </>
            )}
